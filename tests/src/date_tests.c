...
void date_tests(void) {
  ...
  DateTm *t = dateTm_now();
  assert(dateTm_df(dateTm_add(dateTm_add(t, 25), -25), t) == 0);
  assert(dateTm_df(dateTm_add(dateTm_add(t, 25), -30), t) == -5);
  assert(dateTm_df(dateTm_add(dateTm_add(t, 25), -20), t) == 5);
