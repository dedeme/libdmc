...
void str_tests(void) {
  ...
  unsigned *u = NULL;
  u = opt_some(str_to_unicode(""));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("", r));
  u = opt_some(str_to_unicode("a"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("a", r));
  u = opt_some(str_to_unicode("\n"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("\n", r));
  u = opt_some(str_to_unicode("ú"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("ú", r));
  u = opt_some(str_to_unicode("ñ"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("ñ", r));
  u = opt_some(str_to_unicode("€"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("€", r));
  u = opt_some(str_to_unicode("a b"));
  r = ochar_some(str_from_unicode(u));
  assert(str_eq("a b", r));
  u = opt_some(str_to_unicode("a\nb"));
  r = ochar_some(str_from_unicode(u));
